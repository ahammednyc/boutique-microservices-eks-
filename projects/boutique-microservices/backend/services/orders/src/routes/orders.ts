...
router.post('/', async (req, res) => {
  try {
    ...
    const { items, shippingAddress, userId = '00000000-0000-0000-0000-000000000001' } = req.body as CreateOrderRequest & { userId?: string };
...
router.get('/my-orders', async (req, res) => {
  try {
    ...
    const orders = await Promise.all(result.rows.map(async (row: any) => {
      const rawItems = row.items && row.items[0] && row.items[0].id ? row.items : [];
      const items = await Promise.all(rawItems.map(async (item: any) => {
        let product = null;
        try {
          const productResponse = await axios.get(`${PRODUCTS_SERVICE_URL}/${item.productId}`);
          const p = productResponse.data.data;
          product = p ? { ...p, imageUrl: p.imageUrl || p.image_url } : null;
        } catch (e) {
          product = null;
        }
        return { ...item, product };
